...
/**
 * Widget d'aide flottant. Le numéro WhatsApp se règle par
 * NEXT_PUBLIC_WHATSAPP_NUMBER (format international sans « + », ex. 33612345678).
 * Sans numéro configuré, le widget ne s'affiche pas plutôt que d'ouvrir un lien mort.
 */
export function FloatingHelp({ phone }: { phone?: string }) {
  ...
  return (
    <a
      href={`https://wa.me/${phone}?text=${encodeURIComponent("Bonjour, j'ai une question sur AlloChantier")}`}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Poser une question sur WhatsApp"
      className={`fixed bottom-5 right-5 z-[90] flex items-center gap-2 rounded-pill bg-gr px-4 py-3 text-[14px] font-semibold text-w shadow-sh3 transition-all hover:brightness-110 ${
